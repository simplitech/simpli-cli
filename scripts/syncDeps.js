...
const externalVueScopedPackages = {
  '@simpli/test-utils': true,
  '@simpli/eslint-config': true
}
const localPackageRE = /'(@simpli\/(?:cli|eslint|babel)[\w-]+)': '\^([\w-.]+)'/g
...
async function syncDeps ({ local, version, skipPrompt }) {
  ...
  if (!local) {
    ...
    const packages = await globby(['packages/@simpli/*/package.json'])
    const resolvedPackages = (await Promise.all(packages.filter(filePath => {
      ...
      for (const dep in deps) {
        if (dep.match(/^@simpli/) && !externalVueScopedPackages[dep]) {
          continue
        }
        let local = deps[dep]
        if (local.charAt(0) !== '^') {
  ...
  const paths = await globby(['packages/@simpli/**/*.js'])
  paths
    ...
    .forEach(filePath => {
      ...
      const updated = fs.readFileSync(filePath, 'utf-8')
        // update @simpli packages in this repo
        .replace(localPackageRE, localReplacer)
        // also update simpli, vue-template-compiler, vuex, vue-router
...
if (!process.env.SIMPLI_CLI_RELEASE) {
